...
module.exports = function(db, req, res){
    ...
    var calc_dest_key_by_living_area_exept_business_area=functions.calc_dest_key_by_living_area_exept_business_area; 
      ...
      get_building_id_by_apartment(db, apartment_id).then(function(building_id){
        Promise.all(
          [
            ...
            get_house_costs(db, data, building_id, year, "living_area_except_business_area"),
          ])
        .then(function(){
           ...
           var house_costs_distributed_by_living_area_except_business_area =data.house_costs_distributed_by_living_area_except_business_area;
           var now= new Date();
           ...
           var dest_key_by_living_area_exept_business_area = calc_dest_key_by_living_area_exept_business_area(time_period, apartment.size,building.living_area_except_business_area);
           var writeStream;
           ...
           doc.font('Helvetica');
           doc.text("Ihr Verteilerschlüssel ergibt sich aus dem Produkt ihres Anteils der Berechnungsgrundlage sowie den Zeitraum der Abrechnungsperiode, die Sie beansprucht haben dividiert durch das Produkt aus gesamten Berechnungsgrundlage und dem gesamten Zeitraum der Abrechnungsperiode");
           doc.moveDown();
    ...
    var writeDestKeyAndCostTable=(doc, x,y, dest_key, house_costs_object, calc_base, calc_partial, calc_unit)=>{
        ...
        sums.living_area_except_business_arae=sumPartialCosts(dest_key_by_living_area_exept_business_area, house_costs_distributed_by_living_area_except_business_area);
        ...
        if(house_costs_distributed_by_living_area_except_business_area.length>0){
          if(y>600) { doc.addPage(); y=doc.y;}
          doc.font('Helvetica-Bold').text("Berechnung des Verteilerschlüssels anhand der Wohnfläche (Außer Gewerbe):").font('Helvetica');
          doc.moveDown();
          x=doc.x;
          y= doc.y;
          writeDestKeyAndCostTable(doc, x, y, dest_key_by_living_area_exept_business_area, house_costs_distributed_by_living_area_except_business_area, building.living_area_except_business_area, apartment.size, "m²" );
        }
        
        doc.moveDown();
        x=doc.x;
        y= doc.y;
        doc.font('Helvetica-Bold').text("Ihre Grundsteuern:").font('Helvetica');
        doc.text("Grundsteuern zählen als laufende öffentliche Lasten des Grundstücks gemäß §2 Nr. 1 BetrKV zu den umlagefähigen Kosten.");
        doc.moveDown();
        x=doc.x;
        y= doc.y;
        if(y>600) { doc.addPage(); y=doc.y;}
        //doc.font('Helvetica-Bold').text("Grundsteuer:", 72, y).font('Helvetica');
            ...
            y=y+5;
            x=72;
            doc.text(setAsCost(apartment.taxes), x, y);
            x+=150;
            doc.text("365", x,y);
            x+=150;
            doc.text(time_period, x, y);
            x+=120;
            doc.text(setAsCost(sums.taxes), x, y);
            y=y+15;
            doc.moveTo(72, y).lineTo(550, y).stroke();
            y+=5
            doc.text("Gesamtkosten", 72, y);
        ...
        x=72;
        y= doc.y;
        doc.font('Helvetica-Bold').text("Fremdabrechnung:").font('Helvetica');
        doc.text("Verbrauchsabhängige Kosten werden für das gesamte Abrechnungsjahr ermittelt. Die Ablesung dieser Verbrauchswerte und die Berechnung der Heizkosten für das gesamte Abrechnungsjahr werden von einen externen Dienstleister durchgeführt. Ihr Anteil bestimmt sich aus dem Produkt der anteiligen Zeit, des Abrechnungsjahres, die Sie beansprucht haben und der entstandenen Kosten für das gesamte Abrechnungsjahr. Alle von einem externen Dienstleister entstandenen kosten werden zu einer Position zusammengefast.");
        ...
        y= doc.y;
        if(y>600) { doc.addPage(); y=doc.y;}
        doc.font('Helvetica-Bold').text("Fremdabrechnung:", 72, y).font('Helvetica');
            y=y+30;
            x=72;
            doc.text("Gesamtkosten", x, y);
            x+=150;
            ...
            y=y+5;
            x=72;
            doc.text(setAsCost(heating_costs), x, y);
            x+=150;
            doc.text("365", x,y);
            x+=150;
            doc.text(time_period, x, y);
            x+=120;
            doc.text(setAsCost(sums.heating_costs), x, y);
            y=y+15;
            doc.moveTo(72, y).lineTo(550, y).stroke();
            y+=5
            doc.text("Gesamtkosten", 72, y);
           ...
           doc.moveDown();
           doc.moveDown();
           doc.fontSize(11).font("Helvetica").text("Sehr geehrte Mieterin, sehr geehrter Mieter,");
           doc.moveDown();
           ...
           doc.moveDown();
           doc.text("Die Umlage der Betriebskosten erfolgt gemäß den Vereinbarungen in Ihrem Mietvertrag. Bei den Wohnungen wurden zusätzlich die jeweils geltenden gesetzlichen Bestimmungen berücksichtigt. Das sind insbesondere bei freifinanziertem sowie mit vereinbarter Förderung erstelltem Wohnraum die Betriebskostenverordnung vom 25. November 2003 sowie die gesetzlichen Bestimmungen gemäß §§ 556, 556a BGB");
           doc.moveDown();
           if(hasAdditionalPayment==true){
              ...
              doc.font("Helvetica").text("DE83 1203 0000 1009 7974 48", x+150, y);
              y+=15;
              doc.font("Helvetica-Bold").text("BIC:",x, y);
              doc.font("Helvetica").text("BYLADEM1001",x+150, y);
              y+=15;
              ...
              doc.font("Helvetica-Bold").text("Ihr Abrechnungsergebnis ergibt einen Guthaben in Höhe von " + paymentReturn+" €.");
